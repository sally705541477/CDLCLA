...
load Q;   %label matrix
 ...
 load BOW_sift_new_500;
 load bow_1000d;
 Xa_tr = BOW(:,1:2173)';
 Xb_tr = bow_1000d(:,1:2173)';
...
num_class =nClass;
params.dictsize = dictSize;%num_class*20;
%params.Tdata = 7;
%params.dim = 65;
params.iternum = 10;%%%%%%%%%
% params.iterloop = 2;
...
params.fish_tau3 = 50;
params.fish_tau2 = 4;
opts.nClass =  num_class;
opts.dictsize = params.dictsize;
opts.lambda1       = 0.01;%0.3/params.Tdata;
opts.lambda2       = 0.001;
opts.fish_tau3     = params.fish_tau3;
opts.fish_tau2     = params.fish_tau2;
opts.show          = true;
opts.nIter         = params.iternum;

%initdic
% opts.initdic=zeros(1500,200);
% for ii = 1:num_class
%     id = find(train_cat_name==ii);
%     t = floor(length(id)/20);
%     opts.initdic(:,(ii-1)*20+1 : ii*20)=[Xa_tr(:,id(1:t:t*19+1));Xb_tr(:,id(1:t:t*19+1))];
% end

[D,~,~,~,~] = FDDL_modified([Xa_tr;Xb_tr],train_cat_name,opts);
Da = D(1:size(Xa_tr,1),:);
Db = D(size(Xa_tr,1)+1:end,:);
    ...
    for ci = 1:nClass
        ...
        for i = 1 + sub_dictSize * (ci-1) : sub_dictSize * ci
            ...
            Y         =    Xb_tr(:,labels==ci) - ...
                           ...
                           A_b(1+sub_dictSize*(ci-1):sub_dictSize*ci,labels==ci) + Db(:,i)*ai;
            di        =    Y*ai';
            di        =    di./(norm(di,2) + eps);
            Db(:,i)    =    di;
        end
        ...
        for i = 1 + sub_dictSize * (ci-1) : sub_dictSize * ci
            ...
            Y         =    Xa_tr(:,labels==ci) - ...
                           ...
                           A_a(1+sub_dictSize*(ci-1):sub_dictSize*ci,labels==ci) + Da(:,i)*ai;
            di        =    Y*ai';
            di        =    di./(norm(di,2) + eps);
            Da(:,i)    =    di;
        end
